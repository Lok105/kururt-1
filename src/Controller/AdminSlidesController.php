 ...
 */
class AdminSlidesController extends AppController{
    ...
    {
        $slides = $this->paginate($this->AdminSlides);
    ...
    public function add(){
        ...
        $slide = $this->AdminSlides->newEntity();
        if ($this->request->is('post')) {
            $slide = $this->AdminSlides->patchEntity($slide, $this->request->getData());
            ...
            if ($this->AdminSlides->save($slide)) {
                $this->Flash->success('Слайд для главной создан');
    ...
    public function edit($id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            if ($this->AdminSlides->save($slide)) {
                $this->Flash->success('Слайд для главной отредактирован');

                return $this->redirect(['action' => 'index']);
            }
            $this->Flash->error('Ошибка редактирования слайда');
        }
        $this->set(compact('slide'));
        $this->set('_serialize', ['slide']);
    }
    ...
    public function delete($id = null)
    {
        ...
        $slide = $this->AdminSlides->get($id);
        $img_name = $slide->img;
        if ($this->AdminSlides->delete($slide)) {
            if($img_name){
                unlink(WWW_ROOT.'img/upload/'.$img_name);                
            } 
                       
            $this->Flash->success('Слайд удален');
        } else {
