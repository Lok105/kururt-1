...
class ServicesController extends AppController{
    ...
    public function index()
    {
        ...
        $services = $this->Services->find()->where(['parent_id' => '0'])->order(['position' => 'asc']);
    ...
    public function view($alias = null){
        ...
        $service = $this->Services->find()->where(['alias' => $alias])->first();
        //debug($service);
        if(!$service) {
        ...
        if($service->parent_id == 0){
            ...
            $parentOne = $this->Services->find()->select(['id', 'parent_id', 'alias', 'menu_title'])->where(['id' => $service->parent_id])->first();
            if($parentOne->parent_id == 0){
                ...
                $parentTwo = $this->Services->find()->select(['id', 'parent_id', 'alias', 'menu_title'])->where(['id' => $parentOne->parent_id])->first();
        ...
        $services = $this->Services->find()->where(['parent_id' => $service->id])->order(['position' => 'asc']);
