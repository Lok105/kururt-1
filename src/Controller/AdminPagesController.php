...
class AdminPagesController extends AppController{
    ...
    public function index(){
        ...
        if(isset($_GET['search'])){
            ...
            $pages = $this->paginate($this->AdminPages->find()
                ->where([
                    "MATCH(AdminPages.title, AdminPages.alias, AdminPages.h1) AGAINST(:search IN BOOLEAN MODE)" 
                ])->contain()
            ->bind(':search', $search)
            );
            $count_res = 0;
            ...
            $pages = $this->paginate($this->AdminPages);
    ...
    public function add(){
        ...
        $pages_three = $this->AdminPages->getAllPagesThree();
        $page_select = array();
        ...
        $page = $this->AdminPages->newEntity();
        if ($this->request->is('post')) {
            $page = $this->AdminPages->patchEntity($page, $this->request->getData());
        ...
        if(isset($file1['name']) && $file1['name'] != ''){
            ...
            if ($this->AdminPages->save($page)) {
                $this->Flash->success(__('Страница создана.'));

                return $this->redirect(['action' => 'index']);
            }
            $this->Flash->error(__('Ошибка создания страницы.'));
        }
    ...
    public function delete($id = null){
        ...
        if ($this->AdminPages->delete($page)) {
            ...
            $pages = $this->AdminPages->find()->where(['parent_id' => $id]);
            
            foreach($pages as $pp){
                $pp->parent_id = 0;
                $this->AdminPages->save($pp);
