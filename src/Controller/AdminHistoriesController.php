...
class AdminHistoriesController extends AppController{
    ...
    {
        $slides = $this->paginate($this->AdminHistories);
    ...
    public function add()
    {
        $slide = $this->AdminHistories->newEntity();
        if ($this->request->is('post')) {
            $slide = $this->AdminHistories->patchEntity($slide, $this->request->getData());
            ...
            if ($this->AdminHistories->save($slide)) {
    ...
    {
        $slide = $this->AdminHistories->get($id, [
            'contain' => []
        ]);
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            if ($this->AdminHistories->save($slide)) {
                $this->Flash->success('Карточка истории для главной отредактирована');

                return $this->redirect(['action' => 'index']);
            }
            $this->Flash->error('Ошибка редактирования карточки');
        }
        $this->set(compact('slide'));
        $this->set('_serialize', ['slide']);
    }
    ...
    public function delete($id = null)
    {
        ...
        $slide = $this->AdminHistories->get($id);
        $img_name = $slide->img;
        if ($this->AdminHistories->delete($slide)) {
            if($img_name){
                unlink(WWW_ROOT.'img/upload/'.$img_name);                
            } 
                       
            $this->Flash->success('Карточка истории удалена');
        } else {
