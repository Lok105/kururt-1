 ...
 */
class AdminServicesController extends AppController{
    ...
    public function index(){
        ...
        if(isset($_GET['search'])){
            ...
            $pages = $this->paginate($this->AdminServices->find()
                ->where([
                    "MATCH(AdminServices.title, AdminServices.alias, AdminServices.h1) AGAINST(:search IN BOOLEAN MODE)" 
                ])->contain()
            ->bind(':search', $search)
            );
            $count_res = 0;
            ...
            $pages = $this->paginate($this->AdminServices);
        ...
        //Debug($file);
            ...
            if ($info_table->save($information)) {
                $this->Flash->success('Данные основной страницы "Услуги" изменены');
    ...
    public function add(){
        ...
        $pages_three = $this->AdminServices->getAllPagesThree();
        $page_select = array();
        foreach($pages_three as $page_item){
            //debug($page_item);
            $page_select[$page_item->id] = $page_item->title;
            if(!empty($page_item->children)){
                foreach($page_item->children as $child1){
                    $page_select[$child1->id] = '- '.$child1->title;
        ...
        $page = $this->AdminServices->newEntity();
        if ($this->request->is('post')) {
            $page = $this->AdminServices->patchEntity($page, $this->request->getData());
        ...
        if(isset($file1['name']) && $file1['name'] != ''){
            ...
            if ($this->AdminServices->save($page)) {
                $this->Flash->success(__('Услуга создана.'));

                return $this->redirect(['action' => 'index']);
            }
            $this->Flash->error(__('Ошибка создания услуги.'));
        }
        
 
        
        $this->set(compact('page', 'page_select'));
    ...
    public function edit($id = null){
        ...
        $page = $this->AdminServices->get($id, [
            'contain' => []
        ]);
        ...
        //Debug($file);
            ...
            if ($this->AdminServices->save($page)) {
                $this->Flash->success('Услуга отредактирована');
    ...
    public function delete($id = null){
        ...
        if ($this->AdminServices->delete($page)) {
            ...
            //Смотрим и меняем родителя если есть дочерние страницы
            $pages = $this->AdminServices->find()->where(['parent_id' => $id]);
            
            foreach($pages as $pp){
                $pp->parent_id = 0;
                $this->AdminServices->save($pp);
